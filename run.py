...
if __name__ == "__main__":
    ...
    warehouse = Warehouse(
        graphml_path="data/training_validation_data/warehouse/Warehouse_preprocessed.graphml"
    )

    for run in range(20, config["n_runs"]):
        ...
        swarm.run_swarm_task(max_tasks=int(n_robots * 3))
        swarm.stop_run(sensors)

        save_robot_swarm(
            swarm,
            f"data/training_validation_data/swarms_reruns/swarm{run}",
        )
        # swarm.visualization_module.animate_run(
        #     folder_path=f"data/training_validation_data/run_animations/swarm{run}_rerun",
        #     every_nth_img=10,
        # )
